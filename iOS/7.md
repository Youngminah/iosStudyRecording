...
예전에는 QR코드를 인식하려면 , 외부 라이브러리를 사용해야 했으나 iOS 7 이후부터는 

AVFoundation 프레임 워크를 사용하면 된다.

</br>

큐알 코드의 인식과정은 기본적으로 **동영상 캡쳐**로 인식한다. 

아이폰 후면 카메라의 장점은 자동으로 디코딩된 정보를 받아올 수 있다.  (간단)

기본 세팅으로 Privacy - Camera Usage Description 생성하여야 한다.
...
**AVCaptureMetadataOutputObjectsDelegate** : 프로토콜은 모든 메타데이터가 감지되면 가로채는 데 사용됨. 사람이 읽을 수 있는 형식으로 변환.
